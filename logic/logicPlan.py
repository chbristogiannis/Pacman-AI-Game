...
def sentence2() -> Expr:
    ...
    third = ~(b & ~c) >> a
    fourth = ~d >> c
    return conjoin(first, sec, third, fourth)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return PropSymbolExpr(var_str, x, y, now) % disjoin(possible_causes)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in non_outer_wall_coords:
        wall.append(PropSymbolExpr(pacman_str, x, y, t))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            wall.append(PropSymbolExpr(pacman_str, x, y, t))
        KB.append(exactlyOne(wall))

        res = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, t))
        if res:
            return extractActionSequence(model, actions)

        action_list = []
        for action in actions:
            action_list.append(PropSymbolExpr(action, t))
        KB.append(exactlyOne(action_list))

        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorStateAxioms(x, y, t+1, walls_grid))
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        foods = []
        for x, y in food:
            foods.append(~PropSymbolExpr(food_str, x, y, t))
        food_goal = conjoin(foods)
        
        res = findModel(conjoin(KB) & food_goal)
        if res:
            return extractActionSequence(res, actions)

        action_list = []
        for action in actions:
            action_list.append(PropSymbolExpr(dir, t))
        KB.append(exactlyOne(action_list))

        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorStateAxioms(x, y, t+1, walls))
        for x, y in food:   
            a = PropSymbolExpr(pacman_str, x, y, t)
            b = PropSymbolExpr(food_str, x, y, t)
            c = ~PropSymbolExpr(food_str, x, y, t+1)
            d = ~PropSymbolExpr(food_str, x, y, t)
            KB.append(c % (a & b) | d)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        possible_locations = []
        for x, y in non_outer_wall_coords:
            a = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x, y, t))
            b = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x, y, t))
            if not a:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            a = findModel(conjoin(KB) & ~PropSymbolExpr(wall_str, x, y))
            b = findModel(conjoin(KB) & PropSymbolExpr(wall_str, x, y))
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            ...
            c = findModel(conjoin(KB) & ~PropSymbolExpr(pacman_str, x, y, t))
            d = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x, y, t))
            if not c:
                KB.append(PropSymbolExpr(pacman_str, x, y, t))
            elif not d:
                KB.append(~PropSymbolExpr(pacman_str, x, y, t))
